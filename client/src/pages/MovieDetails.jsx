  ...
  return show ? (
    <div className='px-6 md:px-16 lg:px-40 pt-30 md:pt-50'>
      <div className='flex flex-col md:flex-row gap-8 max-w-6xl mx-auto'>
        ...
        <img 
          src={(show?.movie?.poster_path) ? (image_base_url + show.movie.poster_path) : '/vite.svg'} 
        ...
        <div className='relative flex flex-col gap-3'>
          ...
          <h1 className='text-4xl font-semibold max-w-96 text-balance'>{show?.movie?.title || 'Movie Title'}</h1>
          <div className='flex items-center gap-2 text-gray-300'>
            <StarIcon className="w-5 h-5 text-primary fill-primary"/>
            {show?.movie?.vote_average?.toFixed(1) || '0.0'} User Rating
          </div>

          <p className='text-gray-400 mt-2 text-sm leading-tight max-w-xl'>{show?.movie?.overview || 'No description available'}</p>
        ...
        <div className='flex items-center gap-4 w-max px-4'>
          {show?.movie?.casts?.slice(0,12).map((cast,index)=> (
            <div key={index} className='flex flex-col items-center text-center'>
              <img 
                src={(cast?.profile_path) ? (image_base_url + cast.profile_path) : '/vite.svg'} 
                alt={cast?.name || "Actor"} 
                className='rounded-full h-20 md:h-20 aspect-square object-cover'
              />
              <p className='font-medium text-xs mt-3'>{cast?.name || "Actor"}</p>
            </div>
          ))}
        </div>
      </div>
